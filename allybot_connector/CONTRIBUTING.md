...
Ready to contribute? Here's how to set up `allybot-connector` for local development.

1. Fork the `allybot-connector` repo on [GitHub](https://github.com/deliverance-enterprises/allybot-connector).

2. Clone your fork locally:

   ```bash
   git clone git@github.com:{your_username_here}/allybot-connector.git
   ```

3. Install the project in editable mode using [`uv`](https://github.com/astral-sh/uv):

   ```bash
   cd allybot-connector
   uv sync --extra dev
   ```
...
7. Submit a pull request through the [GitHub](https://github.com/deliverance-enterprises/allybot-connector/pulls) website.
...
```bash
reuse annotate --copyright "Deliverance Enterprises" --license "MIT" --recursive . --skip-unrecognised
```
...
> [!IMPORTANT]
> The message of the last commit must match the configured pattern, e.g. "Bump allybot-connector version: 0.1.0 → 0.1.1", for the publish job to run.
