...
"""Entry point for the InOrbit Allybot Connector."""
...
# Local
from allybot_connector import __version__
from allybot_connector.src.config.models import AllybotConnectorConfig
...
def start() -> None:
    ...
    """
    parser = CustomParser(
        prog="allybot-connector",
        description="InOrbit Allybot Connector",
    )
    parser.add_argument(
        "-c",
        "--config",
    ...
    try:
        ...
        config = AllybotConnectorConfig(**yaml_data)
    ...
    connector = AllybotConnector(config)
    LOGGER.info("Starting Allybot Connector...")
    connector.start()
