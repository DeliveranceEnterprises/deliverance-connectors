...
# InOrbit Keenon Connector

[![Python 3.13](https://img.shields.io/badge/python-3.13-blue.svg)](https://www.python.org/downloads/)

InOrbit Edge connector for Keenon

## Overview

This repository contains the [InOrbit](https://inorbit.ai/) Edge Connector for [Keenon](https://example.com).

This integration requires the Connector to be configured following the instructions below.

## API Documentation

This connector implementation is based on the official publicly available Keenon API specifications available on the Keenon support portal. For detailed API documentation and reference materials, please refer to:
...
- Annotation synchronization for waypoint positions between Keenon and InOrbit
...
- Access to a Keenon server with API credentials
- Network connectivity between the connector host and Keenon server
...
- Copy `config/fleet.example.yaml` to `config/my_fleet.yaml` and configure your robot fleet. Each robot needs an InOrbit `robot_id` and the corresponding Keenon `fleet_robot_id`.

- Optionally, configure the Keenon connector-specific settings via environment variables. Copy `config/example.env` to `config/.env` and fill in the values. Any `connector_config` fields can be set using the `INORBIT_KEENON_` prefix (e.g., `INORBIT_KEENON_FLEET_HOST`, `INORBIT_KEENON_FLEET_USERNAME`). Environment variables are used as fallbacks when fields are missing from the YAML configuration. See `config/example.env` for reference. The `.env` file will be automatically loaded when the connector is run.
...
```bash
source config/.env && keenon-connector -c config/my_fleet.yaml
```
...
- **Issues**: [GitHub Issues](https://github.com/deliverance-enterprises/keenon-connector/issues)
