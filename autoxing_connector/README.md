...
# InOrbit AutoXing Cloud Connector

[![Python 3.13](https://img.shields.io/badge/python-3.13-blue.svg)](https://www.python.org/downloads/)

InOrbit Edge connector for AutoXing Cloud

## Overview

This repository contains the [InOrbit](https://inorbit.ai/) Edge Connector for [AutoXing Cloud](https://example.com).

This integration requires the Connector to be configured following the instructions below.

## API Documentation

This connector implementation is based on the official publicly available AutoXing Cloud API specifications available on the AutoXing Cloud support portal. For detailed API documentation and reference materials, please refer to:
...
- Access to a AutoXing Cloud server with API credentials
- Network connectivity between the connector host and AutoXing Cloud server
...
- Copy `config/fleet.example.yaml` to `config/my_fleet.yaml` and configure your robot fleet. Each robot needs an InOrbit `robot_id` and the corresponding AutoXing Cloud `fleet_robot_id`.

- Optionally, configure the AutoXing Cloud connector-specific settings via environment variables. Copy `config/example.env` to `config/.env` and fill in the values. Any `connector_config` fields can be set using the `INORBIT_AUTOXING_CLOUD_` prefix (e.g., `INORBIT_AUTOXING_CLOUD_FLEET_HOST`, `INORBIT_AUTOXING_CLOUD_FLEET_USERNAME`). Environment variables are used as fallbacks when fields are missing from the YAML configuration. See `config/example.env` for reference. The `.env` file will be automatically loaded when the connector is run.
...
```bash
source config/.env && autoxing-cloud-connector -c config/my_fleet.yaml
```
...
- **Issues**: [GitHub Issues](https://github.com/inorbit-ai/autoxing-cloud-connector/issues)
