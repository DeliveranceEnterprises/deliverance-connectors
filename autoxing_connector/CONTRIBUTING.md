...
Ready to contribute? Here's how to set up `autoxing-cloud-connector` for local development.

1. Fork the `autoxing-cloud-connector` repo on [GitHub](https://github.com/inorbit-ai/autoxing-cloud-connector).

2. Clone your fork locally:

   ```bash
   git clone git@github.com:{your_username_here}/autoxing-cloud-connector.git
   ```

3. Install the project in editable mode using [`uv`](https://github.com/astral-sh/uv):

   ```bash
   cd autoxing-cloud-connector
   uv sync --extra dev
   ```
...
7. Submit a pull request through the [GitHub](https://github.com/inorbit-ai/autoxing-cloud-connector/pulls) website.
...
> [!IMPORTANT]
> The message of the last commit must match the configured pattern, e.g. "Bump autoxing-cloud-connector version: 0.1.0 → 0.1.1", for the publish job to run.
