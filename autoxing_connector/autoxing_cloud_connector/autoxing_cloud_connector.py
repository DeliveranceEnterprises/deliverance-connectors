...
"""Entry point for the InOrbit AutoXing Cloud Connector."""
...
# Local
from autoxing_cloud_connector import __version__
from autoxing_cloud_connector.src.config.models import AutoxingFleetConnectorConfig
from autoxing_cloud_connector.src.connector import AutoxingCloudConnector
...
def start() -> None:
    ...
    """
    parser = CustomParser(
        prog="autoxing-cloud-connector",
        description="InOrbit AutoXing Cloud Connector",
    )
    parser.add_argument(
        "-c",
        "--config",
    ...
    try:
        ...
        config = AutoxingFleetConnectorConfig(**yaml_data)
    ...
    connector = AutoxingCloudConnector(config)
    LOGGER.info("Starting AutoXing Cloud Connector...")
    connector.start()
